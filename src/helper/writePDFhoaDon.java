...
//    MauSacBUS mausacBus = new MauSacBUS();
    ...
    public void writeHD(int mahoadon) {
        ...
        try {
            ...
            document.add(header);
            HoaDonDTO pn = HoaDonDAO.getInstance().selectById(mahoadon);
            // Thêm dòng Paragraph vào file PDF

            Paragraph paragraph1 = new Paragraph("Mã hóa đơn: HD-" + pn.getMaHD(), fontNormal10);
            String ncc = KhachHangDAO.getInstance().selectById(pn.getMaKH()).getTenKH();
            ...
            paragraph2.add(new Chunk(createWhiteSpace(5)));
            String diachincc = KhachHangDAO.getInstance().selectById(pn.getMaKH()).getDiaChi();
            paragraph2.add(new Chunk(diachincc, fontNormal10));

            String ngtao = NhanVienDAO.getInstance().selectById(pn.getMaNV()).getTenNV();
            ...
            Paragraph paragraph4 = new Paragraph("Thời gian nhập: " + formatDate.format(pn.getNgayLapHD()), fontNormal10);
            ...
            for (ChiTietPhieuNhapDTO ctp : ChiTietPhieuNhapDAO.getInstance().selectAll(mahoadon)) {
            ...
            Paragraph paraTongThanhToan = new Paragraph(new Phrase("Tổng thành tiền: " + formatter.format(pn.getThanhTienHD()) + "đ", fontBold15));
