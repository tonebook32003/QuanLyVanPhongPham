...
public final class ThongKeDoanhThuTungThang extends JPanel implements ActionListener{
    ...
    public void loadThongKeThang(int nam) {
        ...
        for (int i = 0; i < list.size(); i++) {
            chart.addData(new ModelChart("Tháng " + list.get(i).getThang(), new double[]{list.get(i).getChiphi(), list.get(i).getDoanhthu(), list.get(i).getLoinhuan()}));
        }
        chart.repaint();
        chart.validate();
        pnlChart.add(chart);
        ...
        for (int i = 0; i < list.size(); i++) {
            tblModel.addRow(new Object[]{
                "Tháng " + list.get(i).getThang(), Formater.FormatVND(list.get(i).getChiphi()), Formater.FormatVND(list.get(i).getDoanhthu()), Formater.FormatVND(list.get(i).getLoinhuan())
            });
        }
